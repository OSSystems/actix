...
pub trait WrapFuture<'a, A>
where
    A: Actor,
{
    /// The future that this type can be converted into.
    type Future: ActorFuture<'a, Output = Self::Output, Actor = A>;
    ...
    fn into_actor(self, a: &'a A) -> Self::Future;
}

impl<'a, F: Future + 'a, A: Actor> WrapFuture<'a, A> for F {
    ...
    fn into_actor(self, _: &'a A) -> Self::Future {
        wrap_future(self)
    }
}
...
pub fn wrap_future<'a, F, A>(f: F) -> FutureWrap<F, A>
where
    F: Future + 'a,
{
    FutureWrap {
...
impl<'a, F, A> ActorFuture<'a> for FutureWrap<F, A>
where
    F: Future + 'a,
    A: Actor,
{
